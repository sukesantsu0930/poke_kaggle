ATK_POWERFUL_HAND = 1072
ATK_SUPER_PSY_BOLT = 1071


class Protocol:
    """サブゴール実現の計測: Powerful Hand（または Super Psy Bolt）を最初に打てたターン。
    デッキ設計_フーディン.md の S-0 に対応。"""

    name = "alakazam_first_attack"
    ...
    def observe_before_action(self, obs, action, actor, step):
        ...
        for index in action:
            ...
            if option.attackId in (ATK_POWERFUL_HAND, ATK_SUPER_PSY_BOLT):
    ...
    def observe_after_action(self, previous_obs, action, next_obs, actor, step):
        ...
        for log in next_obs.logs:
            if log.playerIndex == 0 and log.attackId in (ATK_POWERFUL_HAND, ATK_SUPER_PSY_BOLT):
