"""カーズドボム診断 — ボムが実際に何と交換されているかを測る（2026-07-29）。
...
def snapshot(ps):
    ...
    （2026-07-29 実測: 130ダメージで残HP130 が落ちないという矛盾で発覚）。"""
    out = {}
...
def main():
    ...
    for arch in args.opponents.split(","):
        ...
        for _ in range(args.games):
            ...
            # 窓の切り方を必ず間違える（2026-07-29 に3度やった）。ターン開始時の盤面と
            # ターン終了時の盤面だけを比べる。
            cur_turn = None
            turn_start_board = {}
            ...
            while True:
                ...
                # ターン終了と誤認すると**攻撃前の盤面で締めてしまう**（2026-07-29 実測、
